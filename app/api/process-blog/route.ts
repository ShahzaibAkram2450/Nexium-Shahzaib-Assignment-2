...
function generateSummary(content: string): string {
  ...
  // Remove duplicates and filter out short/irrelevant sentences
  ...
  // Take the first 3 most meaningful sentences
...
function extractTextFromHtml(html: string): string {
  ...
  // Remove extra whitespace and repeated lines
  const lines = content
    .replace(/\s+/g, " ")
    .split(/[\r\n]+/)
    .map((line) => line.trim())
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Extract title
    const title =
      $("title").text() || $("h1").first().text() || "Untitled Article";
